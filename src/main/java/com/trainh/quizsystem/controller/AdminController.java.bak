...
public class AdminController {
    ...
    public String permission(Model model, @RequestParam(name = "search", required = false) String search){
        List<Role> roleList = roleServicesImpl.getAllRoleList();
        List<User> userList;
        if(Objects.nonNull(search)){
            userList = userServicesImpl.getUserByLikeUsername(search);
        } else {
            userList = userServicesImpl.getAllUsers();
        }
